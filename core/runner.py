...
class Runner(metaclass=ABCMeta):
    ...
    def __init__(self, cfg: Config):
        ...
        # create model
        self.models = self.build_model(cfg)
    ...
    def build_val_data_loader(self, cfg: Config) -> DataLoader:
        ...
        return [build_data_loader(d, cfg['VAL.DATA']) for d in dataset] 
    ...
    def build_model(self, cfg: Config) -> nn.Module:
        ...
        models = self.define_model(cfg)
        models = [to_device(model) for model in models]
        if torch.distributed.is_initialized():
            models = [DDP(
                model,
    ...
    def save_model(self, epoch: int):
        ...
        model = [model.module if isinstance(model, DDP) else model for model in self.models]
        ckpt_dict = {
            'epoch': epoch,
    ...
    def load_model_resume(self, strict: bool = True):
        ...
        try:
            ...
            if isinstance(self.models, DDP):
                self.model.module.load_state_dict(checkpoint_dict['model_state_dict'], strict=strict)
            else:
                for model, chk in zip(self.models, checkpoint_dict['model_state_dict']):
                    model.load_state_dict(chk, strict=strict)
    ...
    def train(self, cfg: Config):
        ...
        for epoch_index in range(self.start_epoch, self.num_epochs):
            ...
            data_loader = tqdm(zip(*data_loader)) if get_local_rank() == 0 else data_loader
            # data loop
            for iter_index, (data, graph) in enumerate(data_loader):
                loss = self.train_iters(epoch, iter_index, (*data, graph))
                if loss is not None:
                    self.backward(loss)
            # update lr_scheduler
            if self.scheduler is not None:
    ...
    def init_training(self, cfg: Config):
        ...
        # create optim
        self.optim = build_optim(cfg['TRAIN.OPTIM'], self.models)
    ...
    def on_epoch_start(self, epoch: int):
        ...
        sampler = self.train_data_loader[0].sampler
    ...
    def backward(self, loss: torch.Tensor):
        ...
        if self.clip_grad_param is not None:
            for model in self.models:
                torch.nn.utils.clip_grad_norm_(model.parameters(), **self.clip_grad_param)
        self.optim.step()
    ...
    def validate(self, cfg: Config = None, train_epoch: Optional[int] = None):
        ...
        # val loop
        for iter_index, (data, graph) in enumerate(zip(*data_iter)):
            self.val_iters(train_epoch, iter_index, data=(*data, graph))
